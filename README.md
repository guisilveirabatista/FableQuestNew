...
A mini action RPG built with HTML5/JavaScript on top of modern **RPG Maker MZ**
art (48px tiles and charsets), backed by an authoritative Go multiplayer server.
There is no browser build step and no JavaScript package installation.

This is the MZ-reskinned edition of Fable Quest: the RPG Maker 2003 RTP assets
were replaced with RPG Maker MZ library art (the newdata runtime template plus
owned DLC packs). Because MZ art is drawn at 48px instead of 16px, the client
renders into a 3x backing store (`RES = 3` in `client.js`) with smoothing on, so
the painterly MZ tiles, sprites, windowskin and title screen appear crisp at
native resolution while the game logic keeps its 16px tile grid. Assets are
regenerated from the MZ library with `node tools/rip_mz.js` (see that script for
the exact source sheets and the 48px chipset atlas layout, mirrored by the tile
tables in `client.js`).
...
- `midi.js` — a small looping-`<audio>` music player that streams the MZ `.ogg`
  BGM tracks from `assets/bgm/`. It keeps the old `MidiPlayer` API
  (`play/stop/setEnabled/isEnabled`) so the rest of the client is unchanged.
- `assets/` — MZ art and audio generated by `tools/rip_mz.js`: the 48px
  `chipset.png` tile atlas, MZ 48x48 character sheets, the MZ windowskin
  (`system.png`), a title background, the fireball VFX sheet, sound effects
  under `se/` (copied to the names the client uses), BGM under `bgm/`, and the
  MZ default font under `fonts/`. Melee slash/punch effects are drawn
  procedurally in `client.js` (MZ ships combat effects as Effekseer particles,
  not sprite sheets).
...
All art, sound and music are from the **RPG Maker MZ** library — the bundled
runtime resources plus DLC packs (MV Trinity Resource Pack, the Action Combat
plugin sample for the fireball VFX, and the item/weapon icon sets) — and are
subject to their licenses: usable in RPG Maker projects by owners of those
products, not for general redistribution. Don't publish this repo publicly with
the `assets/` folder included.
